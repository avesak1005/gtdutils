...
class GtdUtilityRepository {
  ...
  Future<Result<bool, GtdApiError>> registerDeviceToken({required String userRefcode, required String fcmToken}) async {
    try {
      await utilityResourceApi.registerDeviceToken(userRefcode: userRefcode, fcmToken: fcmToken);
      return const Success(true);
    } on GtdApiError catch (e) {
      Logger.e("registerDeviceToken: $e");
      return Error(e);
    }
  }

  Future<Result<GtdFinancialSumaryRs, GtdApiError>> getAgencySumaryTransactions({required String agencyCode}) async {
    try {
      GtdFinancialSumaryRs result = await utilityResourceApi.getAgencySumaryTransactions(agencyCode);
      return Success(result);
    } on GtdApiError catch (e) {
      Logger.e("getAgencySumaryTransactions: $e");
      return Error(e);
    }
  }

  Future<Result<List<GtdFinancialRs>, GtdApiError>> getAgencyTransactions() async {
    try {
      List<GtdFinancialRs> result = await utilityResourceApi.getAgencyTransactions();
      return Success(result);
    } on GtdApiError catch (e) {
      Logger.e("getAgencyTransactions: $e");
      return Error(e);
    }
  }

  Future<Result<SearchBookingRs, GtdApiError>> getBookingTransactions(SearchBookingRq searchBookingRq) async {
    try {
      SearchBookingRs result = await utilityResourceApi.getBookingTransactions(searchBookingRq);
      return Success(result);
    } on GtdApiError catch (e) {
      Logger.e("getBookingTransactions: $e");
      return Error(e);
    }
  }

  Future<Result<GtdHeaderSummaryRs, GtdApiError>> getHeaderSummary() async {
    try {
      GtdHeaderSummaryRs result = await utilityResourceApi.getHeaderSummary();
      return Success(result);
    } on GtdApiError catch (e) {
      Logger.e("getHeaderSummary: $e");
      return Error(e);
    }
  }
}
