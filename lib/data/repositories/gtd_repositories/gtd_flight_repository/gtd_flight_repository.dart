...
class GtdFlightRepository {
  ...
  Future<Result<List<GtdCountryCodeRs>, GtdApiError>> getCountries() async {
    try {
      var countries = UserManager.shared.countries;
      if (countries.isNotEmpty) {
        return Success(countries);
      }
      final response = await metaApiClient.getcountryCodes();
      UserManager.shared.countries = response;
      return Success(response);
    } on GtdApiError catch (e) {
  ...
  Future<Result<GtdFlightSearchResultDTO, GtdApiError>> searchFlightCombine(FormSearchPayloadModel payload) async {
    try {
      ...
      FilterAvailabilityRq filterRq = FilterAvailabilityRq.createFilterRq(
          searchId: lowSearchDTO.searchId ?? '',
          flightDirection: FlightDirection.d,
          flightType: lowSearchDTO.flightType!);
  ...
  Future<Result<List<AllFilterOptionsDTO>, GtdApiError>> getAllFilterOptions(
    ...
    try {
      ...
      var filterRq = filterOptionsRq.copyWith();
      filterRq.filter = null;
      await airTicketResourceApi.getAllFilterOptions(filterRq).then((value) {
        if (value.success == false) {
          throw GtdApiError(message: "Empty result ??");
        }
  ...
  Future<Result<AddBookingTravellerRs, GtdApiError>> addBookingTraveller(
    ...
    try {
      ...
      // Handle error from draftBookingRs
      if (addBookingTravellerRs.errors?.isNotEmpty == true) {
  ...
  Future<Result<List<BookedFareRule>, GtdApiError>> searchingFlightFareRules(
      String fareSourceCode, String groupId, String searchId) async {
    try {
      List<FareRule> flightFareRulesRs =
          await airTicketResourceApi.searchingflightFareRules(fareSourceCode, groupId, searchId);
      var bookedFareRule = BookedFareRule(fareCode: fareSourceCode, groupId: groupId, fareRules: flightFareRulesRs);
      return Success([bookedFareRule]);
    } on GtdApiError catch (e) {
      Logger.e("searchingFlightFareRules: $e");
      return Error(e);
    }
  }
}
