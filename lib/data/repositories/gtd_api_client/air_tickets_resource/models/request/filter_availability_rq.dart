...
class FilterAvailabilityRq {
  ...
  FilterAvailabilityRq copyWith({
    ...
    return FilterAvailabilityRq(
      searchId: searchId ?? this.searchId,
      filter: filter ?? this.filter,
      departureItinerary: departureItinerary ?? this.departureItinerary,
      sort: sort ?? this.sort,
      page: page ?? this.page,
      size: size ?? this.size,
    );
  ...
  Map<String, dynamic> toJsonQueryParams() =>
      {"page": page, "size": size, "sort": sort?.value, "include-equivfare": false};
...
class FlightFilter {
  ...
  factory FlightFilter.initDefault(FlightDirection flightDirection, {required FlightType flightType}) => FlightFilter(
      cabinClassOptions: [],
      ticketPolicyOptions: [],
      airlineOptions: [],
      stopOptions: [],
...
getFilterOptions(FilterAvailabilityRq? filterAvailabilityRq, List<AllFilterOptionsDTO> filterOptionsDTO,
  ...
  return FlightFilter(
      ...
      step: (flightDirection == FlightDirection.d) ? "1" : "2",
      flightType: filterAvailabilityRq?.filter?.flightType ?? FlightType.dom.value,
