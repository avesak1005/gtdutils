...
enum GtdInputSelectType { none, name, date, nationality, country, passport, membershipCard, city, documentType }
...
abstract class GtdValidateFieldVM {
  ...
  set text(String value) {
    ...
    var offset = value.isEmpty ? 0 : controllerSearch.selection.base.offset;
  ...
  Result<bool, String> validateViewModel() {
    ...
    if (allowValidate) {
      if (!allowEmpty && (value.isEmpty)) {
        ...
        isValid = false;
        if (isEnable == false) {
          errorMess = '${'checkout.pleaseSelect'.tr()} ${label.toLowerCase()}';
        } else {
          errorMess = '${'checkout.pleaseEnter'.tr()} ${label.toLowerCase()}';
        }
        ...
        if (!regex.hasMatch(value)) {
          isValid = false;
          errorMess = '$label ${'checkout.invalid'.tr()}!';
        }
      }
    }
