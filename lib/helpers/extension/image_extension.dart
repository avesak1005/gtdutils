...
extension GtdImage on Image {
  ...
  static Widget imageFromAssetFolder({
    required String assetPathFolder,
    double? width,
    double? height,
    BoxFit? fit,
    Color? color,
  }) {
    String pathExtension = p.extension(assetPathFolder);
    if (pathExtension.toLowerCase() == ".svg") {
      return GtdImage.svgFromSupplier(
        assetName: "assets/$assetPathFolder",
        width: width,
        height: height,
  ...
  static Widget cachedImgUrlWithPlaceholder({
    ...
    var newUrl = url.replaceAll("?ce=0&s=1000x", "");
    return CachedNetworkImage(
      imageUrl: newUrl,
      fit: fit,
