...
extension GtdStringNull on String? {
  ...
  String? formatDateStringFull(String outputPattern) {
    if (isNullOrEmpty()) {
      ...
      try {
        ...
        return DateFormat(outputPattern).format(dateTime.toLocal());
      } catch (e) {
        return null;
      }
    }
  ...
  String? formatDateLocalVNtringFull(String outputPattern) {
    if (isNullOrEmpty()) {
      return null;
    } else {
      var displayTime = dateFormat.parse(this ?? "").localDate(outputPattern);
      return displayTime;
    }
  }
}
